...
#let c1 = rgb("#9fc1c4") // id   (teal)
#let c2 = rgb("#f3b591") // fare (orange)
#let c3 = rgb("#a9cf9c") // tip  (green)
#let c1d = cmain
#let c2d = cdev
#let c3d = rgb("#4a7a5a")
#let skip = rgb("#e9edee") // a skipped (unread) cell
...
#cetz.canvas({
  ...
  let cell(cx, cy, body, fill, tc: cdark, w: 1.25, target: false) = {
    ...
    content((cx, cy), text(size: 8pt, fill: tc, body))
  }

  // --- Logical table (centred on `mid`) --------------------------------------
  let cw = 1.25
  let lx = (mid - cw, mid, mid + cw)
  content((mid, 5.2), text(size: 9pt, weight: "bold", fill: cdark, "Logical table"))
  ...
  // Query cue (centred).
  content((mid, 1.95), text(size: 9pt, weight: "bold", fill: cdev,
  ...
  let read_bracket(i0, i1, ytape, label, col) = {
    ...
    content(((x0 + x1) / 2, yb - 0.32), text(size: 8pt, weight: "medium", fill: col, label))
  }
  ...
  content((mid, 1.35), text(size: 9pt, weight: "bold", fill: cdark, "Row-oriented (CSV)"))
  let row_items = (
  ...
  content((mid, -1.35),
    text(size: 9pt, weight: "bold", fill: cdark, "Column-oriented (Parquet)"))
  let col_items = (
  ...
  content((xpos(1), -1.9 - ch / 2 - 0.45), text(size: 7pt, fill: cgray, "skipped"))
  content((xpos(7), -1.9 - ch / 2 - 0.45), text(size: 7pt, fill: cgray, "skipped"))
  read_bracket(3, 5, -1.9, "read only the 3 fare cells", cdev)
})
