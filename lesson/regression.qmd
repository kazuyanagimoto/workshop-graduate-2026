...
check_question(
  ...
  type = "radio",
  right = quiz_right(
    "アスタリスクは主効果と交互作用の両方を展開します. 交互作用項だけが欲しいときはコロン (x1:x2) を, 算術的な積が欲しいときは I(x1 * x2) を使います."
  ),
  wrong = quiz_wrong(
    "formula の中では * と : は特別な意味を持つのでした. 本文の formula 記法の箇条書きを見直しましょう."
  ),
...
check_question(
  ...
  type = "radio",
  right = quiz_right(
    "fixest は固定効果モデルでは, 最初の固定効果でクラスタリングした標準誤差を既定で返します. lm() や Stata の既定とは違うので, 論文に載せる前に vcov 引数で意図した標準誤差を明示するのが安全です."
  ),
  wrong = quiz_wrong(
    "fixest の既定は lm() とは異なります. 「出力の標準誤差に注目してください」の段落を見直しましょう."
  ),
  button_label = quiz_button,
  random_answer_order = TRUE
)
...
check_question(
  ...
  type = "radio",
  right = quiz_right(
    "縦棒の順序は 外生変数 | 固定効果 | 内生変数 ~ 操作変数 です. 4つ目の選択肢のように操作変数を説明変数に直接入れるのは reduced form であり, educ の係数は得られません."
  ),
  wrong = quiz_wrong(
    "縦棒の順序は 外生変数 | 固定効果 | 内生変数 ~ 操作変数 でした."
  ),
  button_label = quiz_button,
  random_answer_order = TRUE
)
...
check_question(
  ...
  type = "radio",
  right = quiz_right(
    "TWFE は処置済みの個体も比較対象として使うため, 処置効果が時間やコホートで異なると推定量に負の重みがかかりえます. sunab() は Sun and Abraham (2021) の補正済み推定量を計算します. 下の演習で実際に確かめます."
  ),
  wrong = quiz_wrong(
    "問題は標準誤差でも自由度でもなく, 「誰と誰を比べているか」にあります."
  ),
  button_label = quiz_button,
  random_answer_order = TRUE
)
...
この4本を `modelsummary` で整形して, 下の @tbl-exercise-target とまったく同じ表を作るのが課題です.

```{r}
#| label: tbl-exercise-target
#| echo: false
#| tbl-cap: 完成形の回帰表 (これを再現する)
cm_ex <- c(
...
:::: {.quiz}
@tbl-exercise-target を再現してください. 使うのは `coef_map`, `gof_map`, `stars` の3つの引数と, `tinytable` の `group_tt()` です. モデル (1) と (2), (3) と (4) は係数が同じで, 括弧内の標準誤差だけが違う点にも注目してください.

::: {.callout-tip collapse="true"}
## 解答例

```{r}
#| label: exercise-table-solution
cm_ex <- c(
  "flipper_len" = "Flipper length (mm)",
  "bill_len" = "Bill length (mm)"
)
...
1. @sun2021 の補正: `sunab(year_treated, year)` を使う.
...
```{r}
#| label: fig-exercise-es-solution
#| fig-cap: "Event-study estimates on staggered data: naive TWFE versus Sun and Abraham (2021)."
es_twfe <- feols(
