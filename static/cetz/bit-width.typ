...
#let bitfield(x, y, digits, label) = {
  ...
  for (i, d) in digits.enumerate() {
    ...
    content((x + (i + 0.5) * w, y), text(size: 7pt, fill: cdark, str(d)))
  }
  content((x + digits.len() * w + 0.22, y), text(size: 9pt, fill: cdark, "…"))
  content((x + digits.len() * w / 2, y - 0.55), text(size: 7pt, fill: cgray, label))
}
...
#let membar(x0, x1, y, label, open: false) = {
  ...
  if open {
    content((x1 + 0.25, y), text(size: 9pt, fill: cdark, "…"))
  }
  content(((x0 + x1) / 2, y - 0.62), text(size: 8pt, fill: cdark, label))
}
...
#cetz.canvas({
  ...
  // Column headers
  content((-3.4, 3.35), text(size: 8pt, fill: cgray, weight: "bold", "Address"))
  content((2.6, 3.35), text(size: 8pt, fill: cgray, weight: "bold", "Memory (RAM)"))

  // 32-bit row
  content((-6.6, 2.4), anchor: "west", text(size: 10pt, fill: cdark, weight: "bold", "32-bit CPU"))
  bitfield(-4.7, 2.4, (1, 0, 1, 1, 0), "32 bits")
  arr((-2.1, 2.4), (-0.4, 2.4), lc: cgray)
  membar(0, 1.6, 2.4, [$2^32$ addresses = 4 GB])

  // 64-bit row
  content((-6.6, 0.5), anchor: "west", text(size: 10pt, fill: cdark, weight: "bold", "64-bit CPU"))
  ...
  content((0, -0.75), anchor: "west",
    text(size: 7pt, fill: cgray, [not to scale: $2^64$ is 4 billion times $2^32$]))
})
