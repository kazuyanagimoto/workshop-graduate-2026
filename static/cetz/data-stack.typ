...
#cetz.canvas({
  ...
  let mid = 3.5
  let ex1 = 1.6 // Polars centre
  let ex2 = 5.4 // DuckDB centre

  // --- Tiers -----------------------------------------------------------------
  box(mid, 3.0, 7.4, 1.1, "あなたの R コード", "dplyr の動詞: filter() |> summarise()", cbox)
  box(ex1, 1.0, 3.7, 1.1, "Polars", "dataframe library", cnode)
  box(ex2, 1.0, 3.7, 1.1, "DuckDB", "in-process SQL engine", cnode)
  ...
  content((7.45, 3.0), anchor: "west", text(size: 10pt, fill: cgray, "書き方"))
  content((7.45, 1.0), anchor: "west", text(size: 10pt, fill: cgray, "処理エンジン"))
  content((7.45, -1.0), anchor: "west", text(size: 10pt, fill: cgray, "保存形式"))

  // --- Data-flow arrow (disk -> engine -> your result) -----------------------
  arr((-0.45, -1.4), (-0.45, 3.4), lc: cdev)
  content((-0.75, 1.0), std.rotate(90deg,
    text(size: 10pt, weight: "bold", fill: cdev, "データの流れ")))
})
