...
// `textwidth` and `lines-per-page` the way jsbook does. Quarto's stock page.typ
// partial is deliberately replaced by this no-op so that its `us-letter` /
// `1.25in` defaults never fight with the js layout.
$if(logo)$
