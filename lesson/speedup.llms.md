...
[![](../static/cetz/memory-hierarchy.svg)](../static/cetz/memory-hierarchy.svg "Figure 5.1: メモリのヒエラルキー. 上にいくほど高速だが容量は小さい.")

Figure 5.1: メモリのヒエラルキー. 上にいくほど高速だが容量は小さい.
...
Table 5.1: 観測数を変えたときのマッチングの計算時間とメモリ使用量

\\n\\ を 100 から 1000 へ 10 倍にすると, 計算時間は約 100 倍になる一方で, メモリ使用量は約 9 倍にしかなっていないことがわかります. これは, 計算時間が \\O(n^2)\\, 領域計算量が \\O(n)\\ であることと一致しています.
...
[![](../static/img/benchmarks.svg)](../static/img/benchmarks.svg "Figure 5.2: Julia Micro Benchmarks")

Figure 5.2: [Julia Micro Benchmarks](https://julialang.org/benchmarks/)
...
|-----------------------------|-------------|----------------|
| 二重ループ                  | 81.2        | 54.9 KB        |
| 内側ベクトル化 (purrr::map) | 3.6         | 8.4 MB         |
| 完全ベクトル化              | 4.1         | 30.6 MB        |

Table 5.2: match_nn の実装による実行時間とメモリ割り当て (\\n = 1000\\)
...
|-------|-----------|-----------|---------------|
| 1000  | 0.6       | 0.2       | 3.2           |
| 30000 | 542.2     | 122.9     | 4.4           |

Table 5.5: 逐次Rcppと4スレッド並列Rcppの計算時間
