...
これから使うシェルという道具は, 前章の Unix の系譜 ([sec-unix-family](#sec-unix-family)) で見た OS, [Unix](https://ja.wikipedia.org/wiki/UNIX) とともに Bell 研究所で生まれました. macOS と Linux でコマンドや開発ツールがほぼ共通なのは同じ Unix の流れを汲むためで, これから学ぶシェルの操作もこの系譜の上にあります. その設計の背景にあるのが Unix 哲学 (Unix philosophy) です.
...
まずはシェルを開いてみましょう. VSCode を使っているなら, CtrlCtrl + \`\` で統合ターミナルが開きます. macOS のターミナルアプリや, WSL の Ubuntu などでも構いません.
...
パーミッションと所有者については, 後の節で詳しく説明します ([sec-permission](#sec-permission)).
...
[ワイルドカード](https://www.gnu.org/software/bash/manual/html_node/Filename-Expansion.html) は, 他の文字の代わりになる特殊文字です. 重要なのは次の2つです.
...
この章の演習は, クイズではなく実際にターミナルでコマンドを打ちながら進めます. VSCode の統合ターミナル (CtrlCtrl + \`\`) を開いてください. Windows の人は, VSCode で WSL を開いた状態にしておけば, 同じショートカットで Bash のターミナルが開きます.
