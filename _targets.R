...
# --- Data: employment status from e-Stat -----------------------------------
...
fetch_employment_status <- function() {
  ...
  age_lab <- c(
    "03" = "25-29", "04" = "30-34", "05" = "35-39",
    "06" = "40-44", "07" = "45-49", "08" = "50-54"
  )

  # 15+ population and work status (includes the non-employed).
  pop <- estatapi::estat_getStatsData(
    appId = app_id, statsDataId = "0004008100",
    cdArea = "00000", cdCat01 = c("1", "2"), cdCat02 = "0", cdCat04 = "0",
    cdCat03 = ages, cdCat05 = c("0", "1", "2")
  ) |>
    dplyr::transmute(
      sex = dplyr::recode(cat01_code, "1" = "male", "2" = "female"),
      age_group = unname(age_lab[cat03_code]),
      status = dplyr::recode(cat05_code, "0" = "pop", "1" = "employed", "2" = "not_employed"),
      value
    ) |>
  ...
  emp <- estatapi::estat_getStatsData(
    appId = app_id, statsDataId = "0004008150",
    cdArea = "00000", cdCat01 = c("1", "2"), cdCat02 = "0",
    cdCat04 = "00", cdCat05 = "00", cdCat03 = ages, cdCat06 = c("11", "12")
  ) |>
    dplyr::transmute(
      sex = dplyr::recode(cat01_code, "1" = "male", "2" = "female"),
      age_group = unname(age_lab[cat03_code]),
  ...
  wide <- dplyr::left_join(pop, emp, by = c("sex", "age_group")) |>
    ...
    dplyr::select(sex, age_group, regular, nonregular, other_employed, not_employed)
  ...
  dplyr::bind_rows(wide, total) |>
    tidyr::pivot_longer(regular:not_employed, names_to = "status", values_to = "n")
}
...
list(
  ...
  # Employment status by sex and 5-year age from e-Stat (就業構造基本調査 2022).
  # A data-object target; lesson/visualization.qmd draws it via tar_read().
  tar_target(employment_status, fetch_employment_status()),
