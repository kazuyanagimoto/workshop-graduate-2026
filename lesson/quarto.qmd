...
```md
種ごとの平均値を @tbl-penguins-mean にまとめました. 分析に使ったサンプルサイズは `r nrow(penguins)` です.
```

表番号も `tbl-` ラベルで自動管理され, インラインコードの `` `r nrow(penguins)` `` はレンダリング時に実際の行数へ置き換わります. データを差し替えても, 図・表・本文中の数値がすべて自動で追従します.
:::
::::
