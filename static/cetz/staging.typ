...
#cetz.canvas({
  ...
  let title(cy, t) = content((0, cy), text(size: 15pt, weight: "bold", fill: cdark, t))
  let filerow(y, name, col) = content((-2.55, y), anchor: "west",
    text(size: 14pt, fill: col)[#fa-file-lines() #h(5pt) #raw(name)])
  ...
  let flow(y0, y1, lab) = {
    ...
    content((0.3, (y0 + y1) / 2), anchor: "west",
      text(size: 14pt, weight: "bold", fill: cdark, raw(lab)))
  }
  ...
  content((-2.55, -0.8), anchor: "west")[
    #text(size: 14pt, fill: cgray)[#fa-file-lines() #h(5pt) #raw("draft.txt")]
    #h(8pt) #text(size: 11pt, fill: cgray)[(left unstaged)]
  ]
  ...
  cm(-0.85, 5.4, "")
  content((-0.3, 5.4), anchor: "west", text(size: 14pt, fill: cdark, raw("e4f1a9c")))
})
